...
const allCsrPostsQuery = groq`
  *[_type == "csrPost"] | order(publicationDate desc){
    _id,
    title,
    description,
    "images": images[]{
...
export async function getAllCsrPosts(): Promise<CsrPost[]> {
  return client.fetch(allCsrPostsQuery);
}